...
app.get('/api/me', (req, res) => {
  const user = req.session.userId ? db.findUserById(req.session.userId) : null;
...
app.post('/api/upgrade', requireAuth, (req, res) => {
  const user = db.setPremium(req.session.userId, true);
  res.json(publicUser(user));
});
...
io.on('connection', (socket) => {
  ...
  socket.on('join', (room) => {
    ...
    socket.join(room);
    socket.emit('history', db.getRecentMessages(room));
  });
  ...
  socket.on('message', ({ room, text } = {}) => {
    ...
    const msg = db.addMessage(room, user.username, text.trim().slice(0, 500));
